...
namespace
{
	...
	struct Device
	{
		...
		{
			SDL_Color prevCol;
			SDL_GetRenderDrawColor(Renderer, &prevCol.r, &prevCol.g, &prevCol.b, &prevCol.a);
	...
	void DrawRectangle(const Rect& bounding, const Texture* texture, const Color& color, bool doHorizontalFlip, bool doVerticalFlip)
	{
		...
		{
			SDL_Color prevCol;
			SDL_GetRenderDrawColor(CurrentDevice->Renderer, &prevCol.r, &prevCol.g, &prevCol.b, &prevCol.a);
			color.UseAsDrawColor(CurrentDevice->Renderer);
			SDL_RenderFillRect(CurrentDevice->Renderer, &destination);
			SDL_SetRenderDrawColor(CurrentDevice->Renderer, prevCol.r, prevCol.g, prevCol.b, prevCol.a);
		}
		else
		{
